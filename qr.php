...
session_start();
	...
	require_once('../'.$dest.'class/class.order.php');
                ...
                <div class="widget">
                  ...
                  <div class="widget-body">
                    ...
                    <table class="table table-condensed table-striped table-bordered table-hover no-margin">
                      ...
                      <tbody>
					  <?$connection->connect();
					  		$query = mysql_query("SELECT * FROM orders WHERE status='0' ORDER BY dte ASC  ") or die(mysql_error());;
								while($row = mysql_fetch_array($query)) {
									$user_set = new Order($connection);
									$user_set->setOrder($row["id"]);
									?>
									 <tr>
                          ...
                          <td class="hidden-phone center_node">
                            <span class="label label label-<?=$user_set->status==1?'info':'warning';?>">
                               <?='מחכה להדפסה';?>
                            </span>
                          </td>
                          <td class="hidden-phone center_node">
                             <?=date("d-m-Y",strtotime($user_set->dte));?>
                          </td>
                          <td class="hidden-phone center_node">
